...
public final class HibernateUtil2
{
	...
	private static final ThreadLocal<Session> sessionThradLocal = new ThreadLocal<Session>();
	 ...
	 * @author qye.zheng
	 */
	public static final Session getThreadLocalSession()
	{
		Session session = sessionThradLocal.get();
		if (null == session)
		{
			session = getSessionFactory().openSession();
			// 放入本地线程对象
			sessionThradLocal.set(session);
		}
	...
	{
		final Session session = sessionThradLocal.get();
		if (null != session)
		{
			session.close();
			// 清空本地线程缓存
			sessionThradLocal.set(null);
		}
	}
